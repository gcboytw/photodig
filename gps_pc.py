...
SOURCE_DIR = Path(r"Z:\iPhone_2025")
GROUP_DIR = SOURCE_DIR / "group"
GROUP_DIR.mkdir(exist_ok=True)

REFERENCE_IMAGE = SOURCE_DIR / "IMG_5945.JPG"  # ★你要當基準的那張
MAX_DISTANCE = 100  # 公尺，視需要調整
...
def main():
    ...
    ref_coords = get_gps_coords(str(REFERENCE_IMAGE))
    if not ref_coords:
        print(f"❌ 找不到基準圖片 {REFERENCE_IMAGE} 的 GPS")
        return

    print(f"📍 基準座標: {ref_coords[0]:.6f}, {ref_coords[1]:.6f}")
    moved = 0

    # ② 依序處理資料夾裡的每張照片（包括 reference 本身）
    for path in SOURCE_DIR.iterdir():
        if not path.is_file():
            continue
        if path.suffix.lower() not in {".jpg", ".jpeg", ".heic", ".png"}:
